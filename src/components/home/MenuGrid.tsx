...
export default function MenuGrid() {
  return (
    <div
      className="h-full flex min-h-0 flex-col justify-between px-4 py-1 pb-[max(env(safe-area-inset-bottom),4px)] pt-[max(env(safe-area-inset-bottom),4px)]"
      style={{ backgroundColor: "#0B0B0C" }}
    >
      <div className="grid flex-1 min-h-0 grid-cols-3 gap-[clamp(6px,1svh,10px)] py-1 content-evenly">
        {gridEntries.map((entry, i) => {
          ...
          const card = (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              className="flex min-h-[76px] flex-col items-center justify-center gap-[clamp(2px,0.6svh,8px)] rounded-xl px-1 py-2 text-center"
              style={{ backgroundColor: "#1E1C1A", border: "1px solid rgba(184,147,92,0.22)" }}
            >
              <Icon className="h-[clamp(26px,4.2svh,34px)] w-[clamp(26px,4.2svh,34px)]" style={{ color: BRONZE }} strokeWidth={2} />
              <span className="text-[clamp(11.5px,1.65svh,14px)] font-medium leading-tight text-[#F5EFE6]">
                {entry.label}
              </span>
            </motion.div>
          );
      ...
      <div className="mt-1 mb-1 space-y-[clamp(6px,1svh,10px)] flex-shrink-0">
        <motion.a
          ...
          whileTap={{ scale: 0.98 }}
          className="flex items-center gap-2.5 rounded-xl px-3 py-[clamp(8px,1.6svh,14px)]"
          style={{ backgroundColor: "#1E1C1A", border: "1px solid rgba(184,147,92,0.22)" }}
        >
          <span
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[clamp(14px,2svh,17px)] font-semibold text-[#F5EFE6]">
              Chatear con recepción
            </span>
            <span className="block text-[clamp(11.5px,1.55svh,13px)] text-[#D4CCBF]">
